...
public class PrivateMessageListener extends ListenerAdapter {
    ...
    public void onPrivateMessageReceived(final PrivateMessageReceivedEvent event) {
        ...
        if (command != null) {
            ...
            if (language == null) {
				language = new Language("EnglishUK");
			}

            final PendingVerification pendingVerification = Queries.getPendingVerification(user.getId());
            if (pendingVerification != null) {
                final String username = pendingVerification.getUsername();
                final String guild_id = pendingVerification.getGuild_id();
                final Guild guild = Main.getJda().getGuildById(guild_id);
                final String role_id = pendingVerification.getRole();
                Role role = null;
                if (role_id != null) {
					role = guild.getRoleById(role_id);
				}
                final String url = pendingVerification.getSite();
                if (message.equals(username)) {
                    ...
                    try {
                        final String[] parameters = new ParameterBuilder().add("username", username).add("discord_id", user.getId()).build();
                        final Request request = new Request(new URL(url), Main.USER_AGENT, Request.Action.SET_DISCORD_ID, parameters);
                        request.connect();
                ...
                final List<String> guild_ids = Queries.getUserGuilds(user.getId());
                String guild_id;
                if (guild_ids == null) {
					...
					guild_id = guild_ids.get(0);
				}

                try {
                    String api_url = args[0];
                    // TODO Replace all Requests with a new nmc api function i need to make
                    final Request request = new Request(new URL(api_url), "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.0)", Request.Action.INFO);
                    request.connect();
                    final JsonObject response = request.getResponse();
                    if (response.has("nameless_version")) {
                        ...
                        if (Queries.setGuildApiUrl(guild_id, api_url)) {
                            final String guild_name = Main.getJda().getGuildById(guild_id).getName();
